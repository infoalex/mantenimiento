...
class Falla extends ActiveRecord {
     ...
     * @return ActiveRecord
     */
   public function getListadoFalla($order='order.falla.asc', $page='', $sucursal=null) {
        $sucursal = Filter::get($sucursal, 'int');
        
        $columns = 'sucursal.*, falla.* ';
        $join = 'INNER JOIN sucursal ON sucursal.id = falla.sucursal_id  ';        
        $conditions = (empty($sucursal)) ? 'falla.id > 0' : "sucursal.id = '$sucursal'";
        
        $order = $this->get_order($order, 'falla', array('falla'=>array('ASC'=>'falla.falla ASC',
                                                                              'DESC'=>'falla.falla DESC'),
                                                                                'falla_slug',
                                                                                'observacion'));
        if($page) {                
